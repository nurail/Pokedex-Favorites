...
let myJson = (async function main() {
  await fetch(
    ...
    .then((myJson) => {
      ...
      let filterSmall = document.querySelector(".pokedex-filter-height-small");
      let filterMediumHeight = document.querySelector(
        ".pokedex-filter-height-medium"
      );
      let filterTall = document.querySelector(".pokedex-filter-height-tall");
      let filterLight = document.querySelector(".pokedex-filter-weight-light");
      let filterMediumWeight = document.querySelector(
        ".pokedex-filter-weight-medium"
      );
      let filterHeavy = document.querySelector(".pokedex-filter-weight-heavy");
      let newGroupName = document.getElementById("new-group");

      let pokemonDatabaseLength = myJson.length;
      let sortedArray = [];
      let index = 0;
      let globalCounter = 0,
        pokedexFavorites, //Favorite Pokemon Array
        favPokemonCount,
        aToZSearchCounter = 0;
      ...
      function makeGrid(displayCounterArg) {
        if (lowestToHighest.getAttribute("selected") == "selected") {
        ...
        } else if (highestToLowest.getAttribute("selected") == "selected") {
          for (
          ...
          for (
            let c = 0, cNext = c + 1, count = 0;
            c < pokemonDatabaseLength && aToZSearchCounter < displayCounterArg;
            c++, cNext++
          ) {
            count = displayIfSearchExists(c, cNext, count, false); //function call to return count whenever a pokemon is displayed
          }
          ...
          loadMorePokemonContainer.appendChild(loadMorePokemon);
        } else if (advancedSearchBlock.getAttribute("filter") == "active") {
          let count = 0;
      ...
      async function pokemonDisplay(c) {
        ...
        pokemonType2.innerHTML = myJson[c].type[1] ? myJson[c].type[1] : ""; //checking if there's a second type

        pokemon_types_div.appendChild(pokemonType1);
        pokemon_types_div.appendChild(pokemonType2);
        li.appendChild(pokemon_types_div);

        let favorite_button = generateFavoriteButton(c); //favorite button to append
        favPokemonBtn = document.querySelectorAll(".fav-pokemon-btn");

        // console.log("pokedexFavorite", pokedexFavorites);

        let checkForFavorites;
        if (pokedexFavorites) {
          checkForFavorites = pokedexFavorites.fav_pokemon.find((el) => {
            // console.log(el.name, myJson[c].name);
            return el.name == myJson[c].name;
          });
        }
        // console.log("checkForFavorites", checkForFavorites);

        (function checkForFavoritesFn() {
          if (checkForFavorites) {
            favorite_button.style.backgroundColor = "gold";
            favorite_button.setAttribute("filter", "active");
          } else {
            favorite_button.style.backgroundColor = "buttonface";
            favorite_button.removeAttribute("filter");
          }
        })();

        li.appendChild(favorite_button);
      ...
      //function to create favorite button for each pokemon
      function generateFavoriteButton(pokemon_index) {
        let favorite_pokemon = document.createElement("button");
        favorite_pokemon.setAttribute("class", "fav-pokemon-btn");
        favorite_pokemon.innerHTML = "☆";
        favorite_pokemon.classList.add("active");

        //event to update favorite button on click
        favorite_pokemon.addEventListener("click", async (e) => {
          await updateFavPokemon(e); //updates the button element for that pokemon if it exists in that set
          await favPokemonButtonColor(e); //returns list of favorite pokemon for that set
        });

        return favorite_pokemon;
      }

      //function to check for favorites for each set
      async function favPokemonButtonColor() {
        console.log("newGroupName.value", newGroupName.value);
        if (newGroupName.value) {
          ...
          let requestOptions = {
            ...
            body: JSON.stringify(fav_set),
          };

          pokedexFavorites = await fetch(
            ...
            .then((result) => {
              favPokemonCount = 0;
              favPokemonCount = +result.fav_pokemon.length;
              pokedexFavorites = result;
              return result;
            })
            .catch((err) => {
              pokedexFavorites = "";
      ...
      //Event to add favorites on change of input field
      newGroupName.addEventListener("change", () => {
        favPokemonButtonColor();
      });
      ...
      let weaknessFilter = document.querySelectorAll(".filter.filter-weakness");
      ...
      advancedSearchBtn.addEventListener("click", function (e) {
        let filterActiveCheck = advancedSearchBlock.getAttribute("filter");
      ...
      lowestToHighest.addEventListener("click", function lowestToHighestSort() {
        init();
        displayCounter = 16;
        lowestToHighest.setAttribute("selected", "selected");
        highestToLowest.setAttribute("selected", "null");
      ...
      highestToLowest.addEventListener("click", function highestToLowestSort() {
        init();
        displayCounter = 16;
      ...
      advancedSearchToggle.addEventListener(
        ...
        function toggleAdvancedSearch(e) {
          ...
          if (
            e.target.classList.contains("advancedSearchNavigationBar--active")
          ) {
      ...
      //Function to add/remove favorites for each click
      async function updateFavPokemon(event) {
        newGroupName = document.getElementById("new-group");
        console.log("newGroupName", newGroupName.value);

        //Checking for a Set name to add/edit pokemon to
        if (!newGroupName.value) {
          return alert("Give a name for the team first");
        }

        favPokemonBtn = document.querySelectorAll(".fav-pokemon-btn");

        let el = event.srcElement ? event.srcElement : "";

        let favPokemonName = event.srcElement.parentElement
          ? event.srcElement.parentElement
          : "";

        //checking if its an add/remove click by button backgroundColor
        if (el.style.backgroundColor == "buttonface") {
          //To check if count of pokemon in set is <=6
          if (favPokemonCount >= 6) {
            return alert("Remove one favorite before adding another");
          }
          favPokemonCount++;
          el.setAttribute("filter", "active");
          el.style.backgroundColor = "gold";
          favoritePokemonArray = [];
          favoritePokemonArray.unshift(favPokemonName); //adding selected pokemon to an array
          console.log(favoritePokemonArray);
          console.log("Fav pokemon array", favoritePokemonArray);
          console.log(favPokemonName.children[1].childNodes[1].innerHTML);
          console.log(favoritePokemonArray[0].getAttribute("accesskey"));
            ...
            fav_pokemon_sets: [
              {
                ...
                fav_pokemon: [
                  {
                    name: favoritePokemonArray[0].children[1].childNodes[1]
                      .innerHTML,
                    accessKey: Number(
                      favoritePokemonArray[0].getAttribute("accesskey")
                    ),
                  },
                ],
              },
          ...
          console.log("favPokemonJSON", favPokemonJSON);
          ...
          let requestOptions = {
            ...
            headers: myHeaders,
            body: JSON.stringify({
              fav_pokemon: favPokemonJSON,
            }),
          };

          //Fetch API to add favorite pokemon
        ...
        //Remove pokemon on click block
        else {
          favPokemonCount--;
          el.style.backgroundColor = "buttonface";
          el.removeAttribute("filter");
          favoritePokemonArray = el;
            ...
            fav_pokemon_sets: [
              {
                ...
                fav_pokemon: [
                  {
                    name: favoritePokemonArray.parentElement.children[1]
                      .childNodes[1].innerHTML,
                    accessKey: Number(
                      favoritePokemonArray.parentElement.getAttribute(
                        "accesskey"
                      )
                    ),
                  },
          ...
          //Fetch API to remove favorite pokemon
          let response = await fetch(
